...
//	public final static String  sanymenupath_menuid = "sanymenupath_menuid";
	...
	public int doStartTag() throws JspException {	
		...
		String arrowclass = theme == null || theme.equals("admin_1")?"fa fa-circle":"fa fa-angle-right";
		String url =  MenuHelper.getRealUrl(contextpath, MenuHelper.indexurl,MenuHelper.menupath_menuid,publicitem.getId());
				...
				if(ms.size() >= 0)
				{
					...
					for(int i = ms.size() - 1;  i >=0; i --)
					{
						...
						else
						{
							...
							if(m.isUsesubpermission() || StringUtil.isEmpty(m.getUrl()))
								datas.append("<li><i class=\"").append(arrowclass).append("\"></i><span>").append(menu.getName(request)).append("</span></li>");
							else
							{
								url =  MenuHelper.getRealUrl(contextpath, MenuHelper.indexurl,MenuHelper.menupath_menuid,m.getId());
								datas.append("<li><i class=\"").append(arrowclass).append("\"></i><a target=\"_self\" href=\"").append(url).append("\">").append(menu.getName(request)).append("</a></span></li>");
							}
